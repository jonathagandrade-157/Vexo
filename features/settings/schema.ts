 ...
 * arquitetura §10/§18) — os campos da Etapa 4 editáveis depois do
 * onboarding em `/painel/configuracoes`. Sem campo novo: nada de
 ...
 * "parece útil").
 *
 * Fase D2-B.1 — `whatsapp` saiu deste schema: `tenants.whatsapp_phone`
 * agora é editado exclusivamente em `/painel/configuracoes/pedidos`
 * (`features/settings/whatsapp-schema.ts`), nunca aqui — dois formulários
 * gravando a mesma coluna seria a mesma inconsistência que motivou o
 * ajuste (validação fraca aqui, normalização de verdade só do outro
 * lado).
 */
