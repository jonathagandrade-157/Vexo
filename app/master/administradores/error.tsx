...
/**
 * Error boundary padrão do Next.js (App Router) para `/master/administradores`
 * — mesma estrutura de `app/master/lojas/error.tsx`. Uma falha real do
 * Supabase em `listPlatformAdmins` lança (nunca vira `[]` silenciosamente)
 * e cai aqui; nunca expõe `error.message` bruto ao admin.
 */
export default function MasterAdministradoresError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="flex flex-col items-center gap-4 rounded-xl border border-error/30 bg-error-container/10 px-6 py-16 text-center">
      ...
      <div className="flex max-w-md flex-col gap-1">
        <h2 className="font-headline text-headline-sm text-on-surface">Não foi possível carregar os administradores</h2>
